...
I chose to implement the design using a Map and an array because it seemed easier to store data by using the date as a key and an array as the value. A date is a unique combination of numbers, and an array can store multiple elements, which I needed in order to store multiple food items for a single date. I understand that there are other data structures I could use instead of an array, but I chose an array because I am not yet sure which option would be the best.
